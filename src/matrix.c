...
void sumMatrix(double *A, double *B, double *res, int nrow, int ncol){
        ...
        for (row=0; row<nrow; row++){
			...
			if (A[row*ncol + col] == NA || B[row*ncol + col] == NA) res[row*ncol + col] = NA;
		}
        }
...
/*	if ((ncolA > 1000*nrowA) || (nrowB > 1000*ncolB)){
                long int nrtA = nrowA, nctB = ncolB, nctA = ncolA, nrtB = nrowB;
                long int ldc = nrtA, lda = nrtA, ldb = nrtB;
                double alpha = 1, beta = 0;
                tr(B, nrowB, ncolB);
                tr(A, nrowA, ncolA);
                int r = dgemm_("N", "N", &nrtA, &nctB, &nrtB, &alpha, A, &lda, B, &ldb, &beta, res, &ldc);
                tr(res, ncolB, nrowA);
                tr(B, ncolB, nrowB);
                tr(A, ncolA, nrowA);
	} else {
...
int findMin(double *M, int nrow, int ncol){
        ...
        int i, imin = 0;
        for (i=0; i<ncol*nrow; i++)
		if (fabs(M[i]) < fabs(M[imin])) imin = i;
        return imin;
}
...
void scale(double *Matrix, double *rowSd, int nrow, int ncol, int center, int std){
        ...
        if (center){
                ...
                for (i=0; i<nrow; i++){
                        ...
                        if (std){
                                for (j=0; j<ncol; j++){
                                        if(rowSd[i] != 0) Matrix[i*ncol + j] /= rowSd[i];
                                }
                                rowSd[i] = 1;
                        }
                }
